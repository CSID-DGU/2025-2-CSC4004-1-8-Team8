...
const leastSimilar = async (userId, { nodeId, top_k = 10 }) => {
  if (!nodeId) {
    throw new Error('nodeId is required for least_similar recommendation');
  }

  const response = await axios.post(
    `${PYTHON_RECOMMENDATION_URL}?method=least_similar&top_k=${top_k}`,
